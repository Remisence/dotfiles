...
active_text_color="#EFFFF2"
active_bg=
active_underline="#38FF63"

inactive_text_color="#AEC4B2"
inactive_bg=
inactive_underline=
...
raise_or_minimize() {
	if [ "$(get_active_wid)" = "$1" ]; then
		...
		bspc node "$1" --flag hidden=off; wmctrl -ia "$1"
	fi
}
...
generate_window_list() {
	...
	while IFS="[ .\.]" read -r wid ws cname cls host title; do
		...
		printf "%s" "%{A3:$on_click increment_size $wid:}"
		printf "%s" "%{A4:$on_click decrement_size $wid:}"
